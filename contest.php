...
if (isset($_GET['cid'])) {
    ...
    if (!$contest_ok) {
        if($OJ_TEMPLATE == "semantic-ui")
        {
            require("template/semantic-ui/need_contest_password.php");
        }
        else {
        $view_errors = "<h2>$MSG_PRIVATE_WARNING <br><a href=contestrank.php?cid=$cid>$MSG_WATCH_RANK</a></h2>";
        $view_errors .= "<form id='contest_form' method=post action='contest.php?cid=$cid'>$MSG_CONTEST $MSG_PASSWORD:<input id='contest_pass' class=input-mini type=password name=password><input class='btn btn-primary' type=submit></form>";
        $view_errors .= "<script>$('#contest_form').submit(function() {
...
";
    ...
    foreach ($result as $row) {
        ...
        if($privilege||$now>$end_time)
        {
            ...
            $view_problemset[$cnt][1].= $row['pid'] . "<br>Problem &nbsp;" . strval(1001+$cnt);
        }
        else
            $view_problemset[$cnt][1].= "Problem &nbsp;" . strval(1001+$cnt);
        if ($OJ_TEMPLATE != "flat-ui"){
            ...
            if($iscmode)
                $view_problemset[$cnt][2].="Problem &nbsp;" . strval(1001+$cnt);
            else
