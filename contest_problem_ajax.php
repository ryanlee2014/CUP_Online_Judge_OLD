...
$acproblem=$database->select("contest_problem(problem)",[
	...
	$totalproblem=$database->select("contest_problem",[
	  ...
	  foreach($rest as $r)
	  {
	      ...
	      $obj[$cnt]['num']=1001+intval($r['num']);
	      $obj[$cnt]['submit']=$submit;
	      $obj[$cnt]['accept']=$accept;
	      $obj[$cnt]['url']=(strlen(trim($r['oj_name'])) > 0?strtolower(trim($r['oj_name'])):"new")."submitpage.php?cid=".$contest_id."&pid=".$r['num'];
