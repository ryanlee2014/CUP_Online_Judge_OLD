...
if(isset($_GET['cid']))
{
    $sql="SELECT
  msg.num              as pid,
  msg.solution_id      as msid,
  solution.solution_id as csid,
  msg.user_id          as muid,
  solution.user_id     as cuid,
  users.nick           as mnick,
  uss.nick             as cnick,
  msg.sim              as msim
FROM (SELECT *
      FROM (SELECT
              ...
              num
            FROM solution
            WHERE contest_id = ".intval($_GET["cid"]).") solution left join `sim` on sim.s_id = solution.solution_id
      WHERE sim.s_id = solution.solution_id) msg
  left join `solution` on msg.sim_s_id = solution.solution_id
  left join `users` on users.user_id = msg.s_user_id
  left join `users` as uss on uss.user_id = msg.s_s_user_id
WHERE msg.sim_s_id = solution.solution_id and msg.user_id <> solution.user_id";
}
else{
