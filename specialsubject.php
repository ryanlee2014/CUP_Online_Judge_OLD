...
//	$_SESSION['user_id']="Guest";
	...
	if(isset($_GET['tid']))
	{
	   ...
	   // $result=mysqli_query($mysqli,$sql);
		...
		AND `special_subject_problem`.`topic_id`=$tid AND `special_subject_problem`.`oj_name`=`vjudge_problem`.`source`
		union all
    select problem.title as title,
           problem.problem_id as pid,
           'CUP' as source,
           special_subject_problem.num as pnum
  from special_subject_problem,
       problem
  where special_subject_problem.problem_id = problem.problem_id
  and special_subject_problem.topic_id = $tid
  and special_subject_problem.oj_name = 'CUP'
		) problem left join (select problem_id as pid1,count(1) as accepted from vjudge_solution where result=4 group by pid1) p1 on problem.pid=p1.pid1
...
//$result=mysqli_query($mysqli,$sql);
			...
			foreach($result as $row){
			 //   echo var_dump($row);
				...
				$view_problemset[$cnt][2]= "<a href='".$source."submitpage.php?tid=$tid&pid=$cnt' target='_blank'>".$row['title']."</a>";
				}
				else
