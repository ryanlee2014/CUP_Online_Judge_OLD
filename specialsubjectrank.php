...
if($is_vjudge)
{
    $sql="SELECT users.user_id, users.nick, special_subject_problem.num, solution.result
FROM (SELECT user_id,problem_id,result
      FROM `solution`
      WHERE problem_id in(SELECT problem_id FROM special_subject_problem where topic_id = $tid)
        and result = 4
     UNION ALL
