...
$json_type="";
$fingerprint = "";
$privilege = isset($_SESSION['administrator']) || isset($_SESSION['contest_manager']);
if(isset($_POST['json']))
{
...
$submit_is_test_run = ((isset($cid)&&$cid<0)||(isset($id)&&$id<0));
if ($res&&mysqli_num_rows($res)<1&&!$privilege&&!$submit_is_test_run){
...
if(count($rprepend)>0)
{
    $source = mysqli_real_escape_string($mysqli,$rprepend[0]["code"]).$source;
}
$rappend = $database->select("prefile","*",["problem_id"=>$id,"type"=>$language,"prepend"=>0]);
if(count($rappend)>0)
{
    $source = $source.mysqli_real_escape_string($mysqli,$rappend[0]["code"]);
}
/*
...
if(isset($_SESSION['store_id'])) $store_id=$_SESSION['store_id'];
	if ($pid==-1||$mode==0){
	$sql="INSERT INTO solution(problem_id,user_id,in_date,language,ip,code_length,share,judger,fingerprint)
		VALUES('$id','$user_id',NOW(),'$language','$ip','$len','$share','$judger','$fingerprint')";
	}else if($tid!=""||$mode==2){
	$sql="INSERT INTO solution(problem_id,user_id,in_date,language,ip,code_length,topic_id,num,judger,fingerprint)
		VALUES('$id','$user_id',NOW(),'$language','$ip','$len','$tid','$pid','$judger','$fingerprint')";
	}
	else
	{
	    $sql="INSERT INTO solution(problem_id,user_id,in_date,language,ip,code_length,contest_id,num,judger,fingerprint)
		VALUES('$id','$user_id',NOW(),'$language','$ip','$len','$cid','$pid','$judger','$fingerprint')";
	}
